...
def resource_path(relative_path):
    # Get absolute path to resource (handles PyInstaller .exe vs .py)
    try:
...
icon_path = resource_path("Icons8-Ios7-Business-Quill-With-Ink.64.ico")
...
def save_as_note():
    ...
    # Ask the user for a new file name and location
...
def load_notes_list():
    ...
    notes = [file for file in os.listdir("notes") if file.endswith(".txt")]
    if notes:
        for file in notes:
            note_listbox.insert("end", file)
        note_listbox.config(state="normal")
    else:
        note_listbox.config(state="disabled")
...
def import_note():
    ...
    if file_path:
        ...
        dest_path = f"notes/{note_title}.txt"
        count = 1
        while os.path.exists(dest_path):
            dest_path = f"notes/{note_title}_{count}.txt"
            count += 1
        with open(file_path, "r", encoding="utf-8") as src, open(dest_path, "w", encoding="utf-8") as dst:
            dst.write(src.read())
        note_listbox.config(state="normal")  # <-- Enable before loading notes
        load_notes_list()
...
toolbar = tk.Frame(window, bd=1, relief="flat", bg="#B3E0FF")
toolbar.pack(side="top", fill="x")
toolbar = tk.Frame(
    window,
    bd=1,                      # Border width (already set)
    relief="flat",             # Flat or "solid" for a solid border
    bg="#B3E0FF",              # Baby blue background
    highlightbackground="black",   # Border color
    highlightthickness=1           # Border thickness
)
toolbar.pack(side="top", fill="x")

# Load the calculator icon (make sure calculator.png is in your project folder)
calc_icon = tk.PhotoImage(file="Calculator.png")

# Add calculator button to the right of the toolbar, styled like the others
calc_button = tk.Button(
    toolbar,
    image=calc_icon,
    compound="left",  # Show icon to the left of the text
    width=32,        # Adjust width to match other buttons visually
    height=32,        # Adjust height if needed
    bg="#FAFFFF",
    fg="black",
    ...
    padx=0,
    pady=3,
    command=lambda: os.system("calc")
)
calc_button.image = calc_icon  # Prevent garbage collection
calc_button.pack(side="right", padx=0, pady=2)
...
# Import note
import_button = tk.Button(toolbar, text="Import Note", command=import_note)
import_button.pack(side="left", padx=2, pady=2)

#save as
save_as_button = tk.Button(toolbar, text="Save As", command=save_as_note)
save_as_button.pack(side="left", padx=2, pady=2)
...
note_menu.add_command(label="Import Note", command=import_note)
